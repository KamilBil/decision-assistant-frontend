...
const Editor: React.FC<EditorProps> = ({ isNavbarActive, toggleNavbar }) => {
    ...
    const changeTextInNode = (ev: React.MouseEvent<Element, MouseEvent>, node: Node) => {
        setNodeToEdit(node);
        setOpenModalForNode(true);
    }

    const changeTextInEdge = (ev: React.MouseEvent<Element, MouseEvent>, edge: Edge) => {
        setEdgeToEdit(edge);
    ...
    const onFileSelected = useCallback((event: React.BaseSyntheticEvent) => {
    ...
    const onAdd = useCallback((className: string) => {
        const newNode: Node = {
            id: getNodeId(),
    ...
    return (
        ...
        >
            {nodeToEdit && (
                <ModalForNode open={openModalForNode} setOpen={setOpenModalForNode} node_to_edit={nodeToEdit} nds={nodes} setNodes={setNodes} />
            )}
            {edgeToEdit && (
                <ModalForEdge open={openModalForEdge} setOpen={setOpenModalForEdge} edge_to_edit={edgeToEdit} edges={edges} setEdges={setEdges} />
            )}

            <Controls />
            <MiniMap nodeColor={(node) => nodeColor(node)} nodeStrokeWidth={3} zoomable pannable />
            <Background variant={BackgroundVariant.Dots} gap={12} size={1} />
            <Panel position="top-right">
                <Stack spacing={2} flexWrap="wrap">
                    <Button variant="outlined" onClick={() => toggleNavbar(!isNavbarActive)}>
                        {isNavbarActive ? t('Hide navbar') : t('Show navbar')}</Button>
                    <Button color="primary" variant="outlined" onClick={onSave}>{t('Save')}</Button>
                    <Button color="primary" variant="outlined" onClick={onRestore}>{t('Recover')}</Button>
                    <Button color="primary" variant="outlined" onClick={() => onAdd('circle')}>{t('Add a randomness node')}</Button>
                    <Button color="primary" variant="outlined" onClick={() => onAdd('square')}>{t('Add a decision node')}</Button>
                    <Button color="primary" variant="outlined" onClick={() => onAdd('output')}>{t('Add output')}</Button>

                    <Button
                        component="label"
                        variant="contained"
                        startIcon={<CloudUploadIcon />}
                        onChange={onFileSelected} // apply it immediately
                    >
                        {t('Upload')}
